...
/*   Updated: 2025/02/18 00:06:34 by yhajji           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_is_empty(char *map, t_game *game)
{
	...
	while (map[i + 1])
	{
		...
		{
			free(map);
			ft_error_msg("Invalid map. The map has an empty line", game);
		}
		i++;
	}
}
	...
	while (j < game->map.rows)
	{
		...
		while (i < ft_strlen(game->map.map[j]))
		{
			if (game->map.map[j][i] == 'P')
			{
				...
				(1) && (game->exit.x = j, game->exit.y = i);
			else if (game->map.map[j][i] == 'M')
				ft_creat_new_enemy(game, &i, &j);
